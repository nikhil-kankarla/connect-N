...
def get_win_combos(rows = 3, columns = 3, num_consecutive_for_win = 3):
    ...
    while (
        ((outer_perimeter_indices[BOTTOM_ROW_INDX] - outer_perimeter_indices[TOP_ROW_INDX]) >= 1) or
        ((outer_perimeter_indices[RIGHT_COLUMN_INDX] - outer_perimeter_indices[LEFT_COLUMN_INDX]) >= 1)
    ):
